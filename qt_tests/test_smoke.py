...
def test_every_scene_renders_something(shot, scene):
    ...
    # Every scene is now a QDialog the add-on lays out itself, so every one of them
    # carries the "Intern Pearls" title, and this is checked unconditionally. It used
    # to skip QMessageBox scenes, which never round-trip setWindowTitle through
    # windowTitle() (a message box manages its own title, and on macOS an alert gets no
    # title bar at all): About was the first to move to a real dialog, the deck-source
    # choice was the last.
