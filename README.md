...
- **Let me flag problems with cards as they sync**, off by default. The Update my decks screen lists each card a sync would add or change before it's imported. With this off, that list is a quick, read-only preview: no note boxes, nothing to send. Turn it on and a note box appears under each card there, and the run hands back a copyable summary of whatever you flagged, whether or not you go ahead with the update.
...
A picture on a card is named rather than shown while its row is collapsed; opening the row extracts it from the deck file already downloaded and renders it in place, so a long list of pending cards stays cheap to scroll and a card you never open costs nothing to extract.
