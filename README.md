...
- **Codex CLI** (any ChatGPT account, including the free one, which allows roughly 50 agentic coding messages a day; Go, Plus, and Pro allow more): sandboxed read-only, no writes or network. Still worth knowing plainly: read-only means it can open files on your machine and fold what it reads into the cards it drafts, which is not the same guarantee as "can't touch your data."
