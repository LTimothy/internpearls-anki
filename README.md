...
A per-backend turn cap still applies underneath whatever depth is in play, as a safety ceiling rather than something you tune: Quick gets exactly one turn, Thorough up to 15, per generation call and independent of how many cards a reply carries, so a larger automatic card count doesn't also mean a slower or more expensive request. **Thorough** (roughly 1 to 3 minutes) drafts, then self-reviews before handing cards back; **Quick** (roughly 15 to 30 seconds) is a single pass with no self-review. What that means for actually reaching the web is not the same claim on every backend, and the Cards and depth row's own detail line spells out the real one for whichever backend you've picked, not a shared line:
