...
1. Takes a fresh, timestamped backup first (a self-contained `.apkg` with scheduling included, saved internally and pruned to the most recent 10). It covers the decks the run is actually about to change: normally that is just the configured `export_deck`, and a run reaching outside it gets one backup per top-level deck involved instead. Nothing else runs until this succeeds, or you explicitly choose to continue without one.
