...
- **Deck** names the destination deck and which note types are in play (Study Deck - Basic, Study Deck - Cloze; Study Deck - Image ID isn't offered here at all, since its whole field is an image and a generated card has no way to fill that field directly). A Change link opens the same Advanced disclosure.
- **Skills** names exactly what's sent on every run (the bundled InternPearls authoring skill, a deck skill if your configured source offers one and you've consented to it, your own rules if you've set any), with View and Edit my rules links.
