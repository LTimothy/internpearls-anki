...
- **Codex CLI** (any ChatGPT account; the free tier is capped at roughly 50 agentic messages a day): sandboxed read-only, no writes or network. Read-only still means it can read files on your machine and fold them into drafts.
...
**Modes.** Thorough (about 1 to 3 minutes) drafts and then self-reviews; Quick (about 15 to 30 seconds) is one pass. What that means for web access differs by backend, and the Cards and depth row states it for yours:

- **Claude Code**: Quick gets one turn and read access to your attachments only; Thorough gets up to 15 turns and web search.
- **Codex CLI**: always read-only with no network, so Thorough's checking uses local files only. No turn cap.
