...
A card already in your collection can come back with its content rewritten upstream, not only a card you don't have yet, and the confirmation shows both, as rows in one list right on the confirmation itself, not behind a button. Every pending card carries a small chip naming what it is: NEW for a card you don't have yet, UPDATED for one whose content changed upstream (with what each changed field currently says in your collection shown right under it), RETIRED for a card being archived because a split, reword, or removal upstream has replaced it, and MOVED for a card whose deck was reorganized and is being relocated to match. Click a NEW or UPDATED row to open it and read the whole card; RETIRED and MOVED rows are a single line each, since there's nothing more to read out of the collection for either kind, though a RETIRED row also shows a short reason under its identity when the deck source recorded one. The list builds itself in batches as you scroll, so it opens instantly whether a handful of cards are pending or thousands, and your review history stays on every card either way.

Every NEW and UPDATED row also carries its own decision control, next to its header, set to the answer that costs nothing by default. A NEW row gets Import / Skip / Never; an UPDATED row gets Apply / Keep yours. Choosing Skip or Keep yours opens a small feedback box under that card so you can note why, entirely optional; every other row still carries a quiet "Add note" link at the end of the card once you open it, so writing something down is never tied to declining a card. Choosing Never collapses the row to a single struck-through line reading "won't be offered again" and drops the card out of this and every future import; nothing you choose here ever deletes a card already in your collection, and a decline can always be undone later (see Manage decks below). A skipped card comes back the next time its deck changes; a kept card comes back only when that card’s source content changes. A returning card is pre-set to the same choice and wearing a single SKIPPED or KEPT YOURS chip in place of its usual one, with a short "changed since you skipped/kept it" hint under the header if the upstream content has moved since your last active decision on it; leaving the card untouched keeps that hint showing, and it only clears once you actively decline it again (Skip or Keep yours once more), which refreshes what's on file to match the current content. Cards you said Never to don't show up as rows again at all; the run just reports how many are being held back and where to bring one back.

A deck source can also ship a short note with a changed or newly added card, saying why it changed. When that note is reviewer feedback it appears quoted under the card's header, marked "from feedback", so you can see what a rewrite is responding to before choosing Apply or Keep yours; a maintainer's own note appears the same way, unquoted. Feedback from older versions remains underneath as dated "earlier feedback," preserving the context that led to previous revisions even when the card is changed again or the learner skips it repeatedly. Maintainer notes still show only for the exact content they describe. Notes appear on a newly added card only in a deck you already have, since a first sync stays quiet rather than captioning every card with history it doesn't have yet. MOVED rows never carry one, since there's no content left to describe. If a note explains more than one card, or a RETIRED row is the replaced half of a card a note also explains, they're shown together: the note once, as a single explanation, with every row it caused underneath it, rather than the same words repeated on each one.

A changed card offers Apply, Keep yours and Never. Keep yours sets that revision aside and the card is offered again only when its source content changes; Never keeps your version and stops offering changes to that card at all. Neither touches the card you already have, and Never here is reversible from Manage decks → Declined cards, where it is listed apart from cards you never imported.

Skip, Keep yours and Never each open a note box on the click, since those are the decisions worth a word of explanation; Import and Apply leave it shut behind the quiet "Add note" link inside the card's body. Nothing is transmitted either way: notes are collected into a digest at the end of the run for you to copy and send yourself. That digest also includes a current snapshot of every standing Skip, Keep yours, and Never choice—including older runs—so one paste carries both the new decisions and the cards still being held back.
...
Within each deck, labelled cards sort by source label in numeric order (for example, `T10Q01`, `T10Q02`, then `T10Q10`). Unlabelled cards follow in their existing order. Shared change-note groups stay together, ordered by their earliest label, with labelled cards sorted inside each group.
...
Which backends are used, which one is preferred, where each CLI lives, and its default model and effort all live in one place: the AI Backends window. It opens from the wizard itself and nowhere else, since it is only ever about a run the wizard is going to make: a "Set up an assistant" button on the setup page before anything is detected, and a "Setup" link on the input page once a backend has been picked. Each backend gets one compact row: a chip saying what the check found (found, not responding, not found, or ignored if you set it aside), its name and the command it runs, small badges for whether it can take an attached image and, when its account has a free tier, whether that tier is throttled (Antigravity) or just capped (Codex), and one muted line naming the subscription it needs and how restricted it is. That check is a cheap, free `--version` call, so "found" only means the binary runs; it is not proof you're actually signed in. Every row carries an "install guide" link to that tool's own documentation. The preferred backend wears a "preferred" chip; every other row offers a "Use ..." link that makes it the preferred one instead. An "ignore" link sets a backend aside so nothing looks for it again, and reads "use again" afterwards.

Below the rows, one settings panel covers the preferred backend only, since that is the one a run will actually use: its executable path (blank auto-detects; Browse picks one by hand), its Model and Effort controls, and a "Test connection" button. Choose a different backend and the panel follows it. So to set a path for an assistant that has not been found yet, click its "Use ..." link first, then fill in the path in the panel that follows it: the panel only ever belongs to whichever backend is preferred. A backend that has not been found yet has its "Test connection" button disabled entirely, since there's no binary to run a real prompt through. Test connection itself runs one trivial, real prompt and reports only whether it truly answers, "working" or "not working," with a readable message (never the CLI's raw error output) either way; that costs an actual (tiny) model call, so it only ever runs when you click it, never automatically. A Re-check link re-runs every row's check, without touching a backend mid-test, and says alongside it which assistant a run would use.

- **Claude Code** (Claude Pro or Max, no free tier): tools fully restricted. The strongest of the three.
- **Codex CLI** (any ChatGPT account; the free tier is available, capped at roughly 50 agentic coding messages a day, not throttled the way Antigravity's is; Go, Plus, and Pro allow more): sandboxed read-only, no writes or network. Still worth knowing plainly: read-only means it can open files on your machine and fold what it reads into the cards it drafts, which is not the same guarantee as "can't touch your data."
- **Antigravity CLI** (a free, throttled Google account tier; it is the replacement for Gemini CLI, which stopped serving personal Google AI Pro and Ultra accounts on 2026-06-18): the add-on passes `--sandbox` when this binary's own help documents it, and never enables a file-writing tool; headless runs auto-deny a write anyway. Beyond that it relies on the assistant's own approval defaults rather than anything the add-on enforces.
...
- **Deck** names the destination deck and which note types are in play (Study Deck - Basic, Study Deck - Cloze; Study Deck - Image ID isn't offered here at all, since its whole field is an image and a generated card has no way to fill that field directly). Its own detail line says to change it under Advanced, which is where the deck combo and note-type checkboxes actually live.
- **Skills** names exactly what's sent on every run (the bundled InternPearls authoring skill, a deck skill if your configured source offers one and you've consented to it, your own rules if you've set any), with a View link and a second link that reads Add my rules until you've saved any and Edit my rules after.

On Claude Code, a turn cap still applies underneath whatever depth is in play, as a safety ceiling rather than something you tune: Quick gets exactly one turn, Thorough up to 15, per generation call and independent of how many cards a reply carries, so a larger automatic card count doesn't also mean a slower or more expensive request there. Codex CLI and Antigravity CLI have no turn cap in either mode; both are bounded only by the run timeout, as the corrective bullets below spell out. **Thorough** (roughly 1 to 3 minutes) drafts, then self-reviews before handing cards back; **Quick** (roughly 15 to 30 seconds) is a single pass with no self-review. What that means for actually reaching the web is not the same claim on every backend, and the Cards and depth row's own detail line spells out the real one for whichever backend you've picked, not a shared line:
...
Each backend's box in AI Backends also carries a Model field and, for Claude Code, an Effort combo. Claude Code defaults to `sonnet` at `medium` effort in both quality modes: without an explicit model, it would run the signed-in account's own default, the top model for a Max subscriber, which burns credits fast across Thorough mode's up-to-15-turn loop, and `sonnet`/`medium` stays smart enough for drafting cards without that cost; a hand-edited effort value the CLI wouldn't recognize falls back to `medium` too, rather than reaching `claude --effort` and dying with an opaque CLI error. Both fields are editable and persist to config **per backend** as you change them, so a model set while Claude Code is active never pre-fills, or gets sent for, another backend. Codex CLI's Model field is honored too, passed as `--model` only when set and only when the installed CLI's help documents that flag (probed against both `codex --help` and `codex exec --help`, since a subcommand's own options often live only under its own help), so an older Codex CLI without the flag isn't hard-broken by receiving it anyway; this probe is not verified against a real Codex CLI binary, since none is installed in this add-on's own dev environment. Antigravity CLI gets the same treatment plus an Effort combo: `agy` documents `--model` and `--effort` in its own help and lists the ids it accepts under `agy models` (`gemini-3.8-flash-medium` and friends), so both are free choices, sent only when you set one and only when the installed binary's help documents the flag. Leave either blank and Antigravity runs its own default, already a cheap Flash tier. The wizard's input page, once a backend's picked, only shows a one-line summary of all this ("Backend: Claude Code, sonnet, medium effort") and a Setup link back into AI Backends: nothing about enabling, preferring, pathing, modeling, or testing a backend lives on the wizard itself anymore.

Click Generate once the Backend row is ready. The progress page is a status row over a live activity feed. The row is a chip and phase (Drafting, Verify online, Working, and so on, whatever the backend's own events actually name), a muted line with the elapsed time (joined, once you've run a mode a few times, by a learned estimate, the median of your last 10 runs in that mode, e.g. "48s elapsed, your recent Thorough runs averaged 1m 40s."; that's specific to your own machine and material, so there's nothing to show on a first run in a mode, and no estimate is ever invented or guessed), and a trailing Cancel link that actually stops the run. Escape does the same thing as Cancel here, on purpose: it no longer closes the dialog out from under a run in progress. The feed under the row lists what the assistant is doing as it goes (viewed a file, searched the web, ran a command) and, once the reply starts streaming, the row counts the cards drafted so far; Codex reports only finished steps, so its feed is sparser. A run is stopped only after two minutes of silence (three in Thorough), or a 15 or 30 minute ceiling, never by a clock while the assistant is still sending. If a run fails outright, whatever the backend's own raw stream said (every stdout line, stderr, exit code, and how long it ran) is kept in `ai_last_run.log` in the add-on's `user_files` folder, overwritten by the next run, in case the one-line error on screen isn't enough to tell what went wrong. Any line that repeats your material, source, focus text, saved rules, or the prompt itself, is elided from that log rather than written out, so it stays a debugging aid without becoming a second copy of what you pasted in.

Drafted cards come back into a review list, on the same row shape Update my decks uses: a chip with the note type as a small tag underneath it, and an Include/Skip control at the header's right edge, Include by default. The header names what actually happened ("3 cards drafted", plus "from 2 sources" when you attached files, plus how many are included and skipped). Each card flags with a mechanical check (a likely duplicate against your existing collection, invalid cloze syntax, an overlong answer, an image that failed to resolve) before you decide anything, defaulted to Skip accordingly. Choosing Skip reveals a note box under the header, the same one Update my decks uses, for what should change; every card also carries a quiet "Add note" link at the end of its body once you open it, so writing something down is never tied to skipping a card. Edit opens the whole card in one dialog: every field at once, sized to what it typically holds, plus a tags line, with OK writing everything back and re-running the mechanical checks. Revise all sends the whole set back for one more pass carrying every card's note and any free-text feedback: cards you didn't note come back untouched ("keep verbatim"), and the list shows what actually changed. Import writes only the cards left on Include.

Check facts, beside Revise all, sends the current draft back for one more turn asking for a verdict per card rather than a revised set, always in Thorough mode on whichever backend you've picked. Claude Code and Antigravity CLI both have web tools in that mode, so a "Confirmed" verdict there means a page was actually opened; Codex CLI has no network in its sandbox, so its prompt says as much and every card comes back "Unverified: no web access" instead of a confirmation it can't back up. Each card gets one line under its header: Confirmed with the source titles as links (the URL itself is the link's tooltip), Corrected with the assistant's proposed field text shown under your current one and Accept/Keep mine links (nothing changes without a click; Accept edits the card in place the same way a hand edit does), or Unverified with the reason. The header line adds the confirmed/corrected/unverified counts once a check has run. A fresh draft or a revision clears any verdicts from an earlier check, since they describe text that may no longer be on the card.

**No card here ever carries an AI-generated raster image.** An image on a drafted card can only come from three places: a real web source the assistant found and cites, an image file you attached directly, or SVG markup the model draws itself (checked for script content before it's used, and normalized so a root shape that would otherwise render mis-sized in the review row's thumbnail renders correctly). The policy is web-first: with web tools actually in play (Thorough mode, on a backend with web access), a figure of a real thing, such as an ECG or capnography trace, anatomy, radiology, histology, a chemical structure, a device, or a waveform, has to be a real image found online and cited, never drawn; drawing stays available only for a simple schematic when no real image fits, and only as a last resort. Quick mode and a backend with no web access (Codex CLI) skip the figure entirely instead of drawing one, since neither can check a drawn figure is right. Nothing here calls an image-generation model or fabricates a raster picture of anything. Attachments work the same on all three backends: each one reads the scratch copy of exactly the files you attached, and none of them is ever given a file-writing tool. An attached PDF's *text* is always extracted; its embedded images generally are not, since decoding them needs a library (Pillow) Anki's own bundled Python doesn't carry: if that's the case for an attached PDF, the wizard says so once, and the fix is to attach any wanted figures as their own image files instead.

A card carrying any image starts on **Skip**, whatever its mechanical checks say, because you haven't actually seen the picture yet. Every image is resolved right here in the review list, in the background (so the dialog never freezes waiting on a slow download): the row shows a real thumbnail once it's in, and for a web image, the source URL's host, so you can see where the model's suggestion actually points before it ever reaches your collection. A download that fails shows as a check on that card instead of a popup, same as any other mechanical check. Whatever review resolved is exactly what Import writes, so importing never re-downloads or re-reads anything.
...
Each run copies your attachments into a scratch folder in the system temp directory and removes it when the wizard closes. If Anki crashes mid-run that folder is left behind, so the add-on sweeps for its own leftovers shortly after startup: anything of its own older than a day goes, and if what remains still exceeds 200 MB the oldest go too. Nothing outside its own named folders is ever touched.
...
A View link shows exactly what's sent to the assistant on top of your material: the bundled InternPearls authoring skill (card-craft rules that ship with the add-on), plus a deck-specific skill if your configured deck source offers one and you've consented to it, plus your own rules if you've saved any. Nothing from a deck skill is ever sent without that explicit consent, and the same link lets you enable or disable one you've already consented to. With no rules of your own saved, nothing is shown under a "My rules" heading; the view just ends with a line pointing at the wizard's Skills row instead.
...
By default it compares the cards this add-on manages against everything else in your collection, but either side can be pointed at a single deck instead, from the two combos at the top. A filtered deck's cards are always credited to their real home deck, not the filtered deck itself. Rescan runs the comparison again from scratch; it's a background pass over text already in your collection, never a network call, so even a large collection finishes in a few seconds. **Sensitivity** (Strict / Normal / Loose, Normal by default) sets how high a match has to score to show up at all, remembered across sessions and rescanning on change. A cosine score alone can be fooled by a tiny comparison pool, where one shared rare word carries a pair past the threshold on its own; Strict and Normal also require a pair to share at least two informative words (three once either card runs long) that between them carry a real share of the shorter card's own vocabulary, not just the raw score. Loose drops that requirement and goes back to the raw score, for a broader, noisier pass. When either side being compared has fewer than 50 cards, the summary says so plainly rather than letting a thin comparison read as a clean bill.

**Exclude decks**, empty by default, leaves out any note whose home deck path contains one of a comma-separated list of names or partial names (matched case-insensitively), on both sides of the comparison, whichever way each combo is pointed. Typing an entry and moving on (Enter, or clicking elsewhere) rescans only if the text actually changed. The summary line reports "Excluding N deck(s) (M cards)" whenever an entry actually dropped a note, and flags any entry that matches no deck at all ("No deck matches '...'") in the warning colour, so a typo doesn't look identical to it working.

Each candidate is a row: a chip naming its band ("Likely duplicate" or "Possible" at Strict and Normal, the older "Likely duplicate" / "Similar" / "Weak match" score bands at Loose) alongside the score, your card's front in bold, and the other card's front, deck, and note type underneath in muted text, followed by the shared vocabulary that actually drove the match ("shares: fenoldopam, agonist, receptor"). A picture-only front's image name never counts toward that vocabulary, so two unrelated cards that happen to share a figure don't match on the filename alone. A line under the summary explains what the score means. Suspend ours, Suspend theirs, Keep both, and Ignore pair sit as links at the right of the row itself, so they're there whether or not you open it. Opening a row shows both cards' answers side by side. All four are reversible and none destructive: **Suspend ours** and **Suspend theirs** go through Anki's own scheduler, exactly like suspending from Browse, and mark the row so you can see it happened; **Keep both** just closes the row; **Ignore pair** remembers the pair so it never comes back on a future rescan. Copy list puts the whole candidate list on the clipboard as plain text, one line per pair.
