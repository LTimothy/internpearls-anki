...
- **Let me flag problems with new cards as they sync**, off by default. Update my decks lets you preview each card a sync would add or change before it's imported. With this off, that preview is a quick, read-only list: no note boxes, nothing to send. Turn it on and a note box appears under each card, and closing the preview offers a copyable summary of whatever you flagged, whether or not you go ahead with the update.
